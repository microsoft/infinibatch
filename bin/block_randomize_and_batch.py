...
# simple command-line wrapper around BucketedReadaheadBatchIterator on a ChunkedDataset
# Example:
...
batch_labels = 500
bg = BucketedReadaheadBatchIterator(ds, read_ahead=100, key=lambda line: len(line), batch_size_fn=lambda line: batch_labels // (1+len(line)), seed=1)
for batch in bg:
    print(f"\n---- size {len(batch)} ---\n")
    print("\n".join(batch))
