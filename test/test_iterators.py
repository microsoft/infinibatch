...
class Testchunked_dataset_iterator(TestBase):
    def test_no_shuffle(self):
        items = list(itertools.islice(chunked_dataset_iterator(self.chunk_file_paths, readlines_from_zipped, shuffle=False, buffer_size=1000), len(self.flattened_test_data)))
        self.assertListEqual(items, self.flattened_test_data)
    ...
    def test_transform(self):
        ...
        items = list(itertools.islice(chunked_dataset_iterator(self.chunk_file_paths, readlines_from_zipped, shuffle=False, buffer_size=1000, transform=transform), len(self.flattened_test_data)))
        self.assertListEqual(items, modified_test_data)

    def test_two_instances(self):
        dataset0 = chunked_dataset_iterator(self.chunk_file_paths, readlines_from_zipped, shuffle=False, buffer_size=1000, num_instances=2, instance_rank=0)
        dataset1 = chunked_dataset_iterator(self.chunk_file_paths, readlines_from_zipped, shuffle=False, buffer_size=1000, num_instances=2, instance_rank=1)
    ...
    def test_checkpointing(self):
        ...
        for use_block in (True, False):
            for i in range(2):
                ...
                dataset = chunked_dataset_iterator(self.chunk_file_paths, readlines_from_zipped, shuffle=(i % 2 == 0), buffer_size=1000, seed=i, num_instances=2, instance_rank=0, use_block=use_block)
                for _ in range(first_length):
                    next(dataset)
                checkpoint = dataset.getstate()
                items1 = list(itertools.islice(dataset, extra_length))
...
class TestBucketedReadaheadBatchIterator(TestBase):
    def txest_basic_functionality(self):
        ...
        bg = BucketedReadaheadBatchIterator(
            chunked_dataset_iterator(self.chunk_file_paths, readlines_from_zipped, shuffle=True, seed=1),
